...
export function BitstackImport({ wallets }: { wallets: WalletOption[] }) {
  ...
  const preview = useMemo(
    () => (text ? parseBitstackCsv(text) : null),
    [text],
  );
          ...
          <div className="rounded-lg border border-black/[.08] bg-zinc-50 p-3 text-sm dark:border-white/[.1] dark:bg-zinc-900">
            ...
            {preview.skipped > 0 ? (
              <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
                Les retraits crypto (transferts vers un wallet externe) et
                les dépôts en euros ne sont pas des achats/ventes — ils sont
                ignorés.
              </p>
            ) : null}
          </div>
