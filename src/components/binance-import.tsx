...
export function BinanceImport({ wallets }: { wallets: WalletOption[] }) {
  ...
  const preview = useMemo(
    () => (text ? parseBinanceCsv(text) : null),
    [text],
  );
  ...
  return (
    <form action={formAction} className={`${ui.card} flex flex-col gap-4`}>
      ...
      <label className={ui.label}>
        Fichier Binance — Historique des transactions (.csv)
        <input
          type="file"
          accept=".csv,text/csv"
          onChange={handleFile}
        ...
        <>
          <div className="rounded-lg border border-black/[.08] bg-zinc-50 p-3 text-sm dark:border-white/[.1] dark:bg-zinc-900">
            ...
            <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
              Binance n&apos;exporte pas les prix : seuls les achats/ventes
              contre euro ou stablecoin (USDC, USDT…) sont importés. Les
              conversions crypto↔crypto, dépôts et retraits sont ignorés. Les
              montants en stablecoin sont traités en dollars.
            </p>
          </div>
